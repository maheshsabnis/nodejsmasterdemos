...
var productModel = mongoose.model("ProductsData", productsSchema, "ProductsData");
...
instance.get("/api/products/:id", function(request, response) {
  ...
  console.log("Received id to search record =" + id);
  productModel.findOne({_id:id},function(err,res) {
    if (err) {
      response.statusCode = 500;
      response.send(err);
    } else {
...
instance.put("/api/products/:id", function(request, response) {
  ...
  console.log(JSON.stringify(prd));
  productModel.findOneAndUpdate({_id:id},prd, function(err,doc, res) {
    if (err) {
      response.statusCode = 500;
      response.send(err);
    } else {
      console.log(`After update record is ${JSON.stringify(doc)}`)
      response.send({ status: 200, data: doc });
    }
...
instance.delete("/api/products/:id", function(request, response) {
  ...
  productModel.deleteOne({_id:id},function(err){
     if(err){
      ...
      response.send({ status: 200, data: 'Record Deleted Successfully.' });
     }
  });
  // respond array
});
