...
instance.get("/api/products", function(request, response) {
  ...
  var promise = authLogic.checkUser(user);
  promise.exec(function(err, res) {
    if (err) {
      console.log("err");
    } else {
      if (res.length > 0) {
        if (
          ...
          console.log("in if success" + JSON.stringify(res));
        ...
        response.send({ status: response.statusCode, error: "UnAuthorized" });
      }
    }
  });
});
